...
async function gatherStatisticalReleases(year, opts) {
  ...
  try {
    ...
    out.push({ label: 'CAO Japan', source: '(error)', error: String(err.message || err), eventIds: [] });
  }
  // Japan Customs — Trade Statistics (貿易統計速報).
  try {
    const r = await customs_jp.getReleases(year, opts);
    for (const rel of r.releases) {
      out.push({
        label: rel.label,
        source: r.source,
